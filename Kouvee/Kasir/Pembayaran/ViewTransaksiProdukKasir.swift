//
//  DetailTransaksiProdukKasir.swift
//  Kouvee
//
...
import UIKit
import Alamofire
class ViewTransaksiProdukKasir: UIViewController,UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
   ...
   /* @IBAction func btnAdd(_ sender: Any) {
    ...
    @IBAction func btnSelesai(_ sender: Any) {
      //  performSegue(withIdentifier: "editTransaksiProdukSegue", sender: Any?.self)
        let alert = UIAlertController(title: "Lakukan Pembayaran ? ", message: TransaksiP?.id_transaksi_produk, preferredStyle: .alert)
        ...
        alert.addAction(UIAlertAction(title: "Simpan", style: .default, handler: {ACTION in
            ...
            if textFieldDiskon.text == ""{
                self.pembayaran(id_cs: self.TransaksiP!.id_pegawai,
                id_kasir: Info.sharedInstance.id_pegawai,
                id_hewan: self.TransaksiP!.id_hewan,
                diskon: "0",
                status: "1");
            }
...
fileprivate func update(id_produk: String, jumlah_produk: String, id_detil:String){
    ...
    fileprivate func pembayaran(id_cs: String, id_kasir: String, id_hewan: String, diskon:String , status: String){
        ...
        guard let url = URL(string: urlTrans + self.TransaksiP!.id_transaksi_produk ) else { return }
            ...
            do{
                ...
                if Message != "Data Berhasil Di Ubah" {
                    ...
                    DispatchQueue.main.async {
                        ...
                        alert.addAction(UIAlertAction(title: "Tutup", style: .default, handler: {(UIAlertAction) in
                            guard let url = URL(string: self.urlNota + self.TransaksiP!.id_transaksi_produk) else { return }
