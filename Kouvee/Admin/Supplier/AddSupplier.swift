...
var urlSupplier = "http://www.kouvee.xyz/index.php/Supplier"
        ...
        if txtNomor.text!.count != 12{
            let alert = UIAlertController(title: "Form Invalid", message: "Nomor Telepon Harus 12 Digit", preferredStyle: .alert)
                ...
                alert.addAction(UIAlertAction(title: "Ok", style: .default, handler:{ (UIAlertAction) in
                   ...
                   self.post(namasupplier: self.txtNama.text!,             alamat: self.txtAlamat.text!,
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "back"{
            guard let VC = segue.destination as? FetchSupplier else {return}
...
fileprivate func post(namasupplier: String, alamat: String, nomor: String, id_pegawai: String){
    ...
    self.request = Alamofire.request(urlSupplier, method: .post, parameters: parameters)
