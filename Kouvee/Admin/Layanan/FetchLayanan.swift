...
class FetchLayanan: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UISearchBarDelegate{
   ...
   private let refreshControl = UIRefreshControl()
    ...
    @IBAction func btnSort(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "ID", style: .default, handler: {ACTION in
            self.Layanans.sort(by: {$0.id_layanan.localizedStandardCompare($1.id_layanan) == .orderedAscending})
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        performSegue(withIdentifier: "ViewLayananSegue", sender: indexPath)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ViewLayananSegue"{
            guard let VC = segue.destination as? ViewLayanan else {return}
            let indexPath = sender as! IndexPath
            if(filtered.count > 0){
                let Serv = self.filtered[indexPath.row]
                VC.Layanans = Serv
            }else{
                let Serv = self.Layanans[indexPath.row]
                VC.Layanans = Serv
            }
        }
